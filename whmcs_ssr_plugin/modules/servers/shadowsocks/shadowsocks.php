...
function shadowsocks_ConfigOptions() {
	...
	"混淆" => array("Type" => "textarea"),
	"协议" => array("Type" => "textarea"),
	);
...
function shadowsocks_ClientArea($params) {
	...
	$node2 = substr($node,0,-4);
$urlt = "{$params['configoption6']}:{$params['configoption7']}:{$params['configoption2']}:{$password}@{$node2}:{$Port}";
$orginurlt = "{$params['configoption2']}:{$password}@{$node2}:{$Port}";
$urlt = base64_encode($urlt);
    	...
    	$html = "
				<table class=\"table table-bordered table-hover tc-table\">
					<tbody>
						...
						<tr>
	                       <td>混淆</td><td>{$params['configoption6']}</td>
					    </tr>
						<tr>
